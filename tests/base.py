...
from blueprints import Graph, VERTEX, EDGE
...
class BaseTestGraph(TestCase):
    ...
    def test_create_edge(self):
        with self.graph.transaction():
            ...
            edge = self.graph.edge(vertex, other, 'link')
    ...
    def test_vertex_outgoings_with_label(self):
        with self.graph.transaction():
            vertex = self.graph.vertex()
            other = self.graph.vertex()
            link = self.graph.edge(vertex, other, 'link')
            self.graph.edge(vertex, other, 'edge')
    ...
    def test_iter_edges(self):
        with self.graph.transaction():
            ...
            self.graph.edge(vertex, other, 'link')
            self.graph.edge(other, moar, 'link')
            self.graph.edge(moar, vertex, 'link')
        num = 0
    ...
    def test_create_vertex_index(self):
        index = self.graph.index.create('foo', VERTEX)
        self.assertTrue(index)

    def test_create_edge_index(self):
        index = self.graph.index.create('foo', EDGE)
        self.assertTrue(index)
